...
rm(list=ls())
library(ggplot2)
setwd("C:/Users/kaylee/Documents/Kaylee_Stuff/Smith_Lab/Data/Mouse_Validation_Data/Maternal_IL6_4.5gDose")

#reads a file in a table format and creates a data frame from it
datafile <- "MatLivIL6_4.5gkgDose_Ctcalcs_woutOutliers_file1_For_ddCtPlot.csv"
...
png("MatLiv_IL6_ddCtDotPlot_4.5gDose_woutOutliers.png", units="in", width=7, height=7, res=1000)
...
p <- ggplot(data= dataframe_AdjCtCalcs, aes(x=Biological.Sets, y=Adj_Ct, color = Biological.Sets), fill=order) + #creates original graph
  ...
  ggtitle("Maternal Liver IL-6") + #adds title
  xlab("Treatment") + #adds x-axis label
  ylab("Maternal Liver Il6/Gapdh expression relative to MD") + #adds y-axis label
