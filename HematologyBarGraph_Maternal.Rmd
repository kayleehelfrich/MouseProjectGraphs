---
title: "Maternal Mouse Hematology Data AIN93G"
author: "Kaylee Helfrich"
date: "2/19/2020"
output: 
  pdf_document: default
  html_document: default
data: AIN93G_Maternal_Mouse_Hematology_CLEAN.csv
---
...
# Graphing of CBCs
This document details my graphing of the hematology data from maternal CBCs. These litters were collected in the fall of 2019 by Brandon Presswood, Dane Ricketts, Olivia Rivera, and Katie Walter. The design was the standard design that we use, with 3.0g/kg ethanol from GD8.5-17.5 or 80% isocaloric maltodextrin. All dams consumed AIN-93G diet from 3 weeks prior to pregnancy through pregnancy and collection. Tissues were harvested on GD17.5, 4 hours after the final dose. Blood was collected from the mothers by heparin-coated cardiac puncture. The blood was run using the diluted mode on the pochi analyzer. All graphs are mean with standard deviation, since there are not multiple measures per dot.
...
# White Blood Cells
## All code is included for this first example, but thereafter, only the heading and graphs are shown. For all graphs, all data is graphed unless mentioned otherwise. The warnings and messages were excluded from the print out, because they are simply warning that some NA's were left out of the graphs. The NA's were included to make the number of samples in each treatment group even, and it is not a problem that they exist.
...
#CombineTables#
dataframe_Mat_WBCs_Combined <- rbind(MD_WBCs, Alc_WBCs)

#Make Graph#
png("Maternal_WBCs_Dotplot.png", units="in", width=7, height=7, res=600) 

sets <- c("MD", "PAE")

p<- ggplot(dataframe_Mat_WBCs_Combined, aes(x=Group, y=Mean)) + 
  ...
  ggtitle("Maternal WBCs") +
  ylab(expression("Maternal WBCs (x10^3 cells/uL)")) + 
...
dev.off() 
```
```{r White Blood Cells, echo = FALSE, warning=FALSE, message=FALSE}
print(p)
```
...
#CombineTables#
dataframe_Mat_RBCs_Combined <- rbind(MD_RBCs, Alc_RBCs)

#Make Graph#
png("Maternal_RBCs_Dotplot.png", units="in", width=7, height=7, res=600) 

sets <- c("MD", "PAE")

p<- ggplot(dataframe_Mat_RBCs_Combined, aes(x=Group, y=Mean)) + 
  geom_bar(stat="identity", color="black", aes(fill = Group),
           position=position_dodge()) +
  scale_fill_manual(values=c("MD" = "#0000CC", "PAE" = "#6699FF")) +
  geom_errorbar(aes(ymin=Lower, ymax=Upper), width=.2,
...
#CombineTables#
dataframe_Mat_HGB_Combined <- rbind(MD_HGB, Alc_HGB)

#Make Graph#
png("Maternal_Hemoglobin_Dotplot.png", units="in", width=7, height=7, res=600) 

sets <- c("MD", "PAE")

p<- ggplot(dataframe_Mat_HGB_Combined, aes(x=Group, y=Mean)) + 
  geom_bar(stat="identity", color="black", aes(fill = Group),
           position=position_dodge()) +
  scale_fill_manual(values=c("MD" = "#0000CC", "PAE" = "#6699FF")) +
  geom_errorbar(aes(ymin=Lower, ymax=Upper), width=.2,
...
#CombineTables#
dataframe_Mat_HCT_Combined <- rbind(MD_HCT, Alc_HCT)

#Make Graph#
png("Maternal_Hematocrit_Dotplot.png", units="in", width=7, height=7, res=600) 

sets <- c("MD", "PAE")

p<- ggplot(dataframe_Mat_HCT_Combined, aes(x=Group, y=Mean)) + 
  geom_bar(stat="identity", color="black", aes(fill = Group),
           position=position_dodge()) +
  scale_fill_manual(values=c("MD" = "#0000CC", "PAE" = "#6699FF")) +
  geom_errorbar(aes(ymin=Lower, ymax=Upper), width=.2,
...
#CombineTables#
dataframe_Mat_MCV_Combined <- rbind(MD_MCV, Alc_MCV)

#Make Graph#
png("Maternal_MeanCellVolume_Dotplot.png", units="in", width=7, height=7, res=600) 

sets <- c("MD", "PAE")

p<- ggplot(dataframe_Mat_MCV_Combined, aes(x=Group, y=Mean)) + 
  geom_bar(stat="identity", color="black", aes(fill = Group),
           position=position_dodge()) +
  scale_fill_manual(values=c("MD" = "#0000CC", "PAE" = "#6699FF")) +
  geom_errorbar(aes(ymin=Lower, ymax=Upper), width=.2,
...
#CombineTables#
dataframe_Mat_MCH_Combined <- rbind(MD_MCH, Alc_MCH)

#Make Graph#
png("Maternal_MeanCellHemoglobin_Dotplot.png", units="in", width=7, height=7, res=600) 

sets <- c("MD", "PAE")

p<- ggplot(dataframe_Mat_MCH_Combined, aes(x=Group, y=Mean)) + 
  geom_bar(stat="identity", color="black", aes(fill = Group),
           position=position_dodge()) +
  scale_fill_manual(values=c("MD" = "#0000CC", "PAE" = "#6699FF")) +
  geom_errorbar(aes(ymin=Lower, ymax=Upper), width=.2,
...
#CombineTables#
dataframe_Mat_MCHC_Combined <- rbind(MD_MCHC, Alc_MCHC)

#Make Graph#
png("Maternal_MeanCellHemoglobinConcentration_Dotplot.png", units="in", width=7, height=7, res=600) 

sets <- c("MD", "PAE")

p<- ggplot(dataframe_Mat_MCHC_Combined, aes(x=Group, y=Mean)) + 
  geom_bar(stat="identity", color="black", aes(fill = Group),
           position=position_dodge()) +
  scale_fill_manual(values=c("MD" = "#0000CC", "PAE" = "#6699FF")) +
  geom_errorbar(aes(ymin=Lower, ymax=Upper), width=.2,
  ...
  ylab(expression("Maternal MCHC (g/dL)")) + 
  theme(plot.title = element_text(hjust = 0.5, size=18), 
        axis.title.x = element_blank(), #gets rid of x-axis label
        axis.title.y = element_text(size = 16),
        panel.grid.major = element_blank(), #gets rid of major gridlines
...
#CombineTables#
dataframe_Mat_PLT_Combined <- rbind(MD_PLT, Alc_PLT)

#Make Graph#
png("Maternal_Platelets_Dotplot.png", units="in", width=7, height=7, res=600) 

sets <- c("MD", "PAE")

p<- ggplot(dataframe_Mat_PLT_Combined, aes(x=Group, y=Mean)) + 
  geom_bar(stat="identity", color="black", aes(fill = Group),
           position=position_dodge()) +
  scale_fill_manual(values=c("MD" = "#0000CC", "PAE" = "#6699FF")) +
  geom_errorbar(aes(ymin=Lower, ymax=Upper), width=.2,
  ...
  geom_point(aes(x=Group, y=Value),
             size = 1.25, shape = 19, position = position_jitter(width=.15, height=0.1)) +
  scale_y_continuous(expand = c(0,0), limits = c(-100,2000)) +
  ggtitle("Maternal Platelets") +
  ylab(expression("Maternal PLT (x10^3 platelets/uL)")) + 
  theme(plot.title = element_text(hjust = 0.5, size=18), 
        axis.title.x = element_blank(), #gets rid of x-axis label
        axis.title.y = element_text(size = 16),
        panel.grid.major = element_blank(), #gets rid of major gridlines
...
#CombineTables#
dataframe_Mat_ZnPP_Combined <- rbind(MD_ZnPP, Alc_ZnPP)

#Make Graph#
png("Maternal_ZincProtoporphyrin_Dotplot.png", units="in", width=7, height=7, res=600) 

sets <- c("MD", "PAE")

p<- ggplot(dataframe_Mat_ZnPP_Combined, aes(x=Group, y=Mean)) + 
  geom_bar(stat="identity", color="black", aes(fill = Group),
           position=position_dodge()) +
  scale_fill_manual(values=c("MD" = "#0000CC", "PAE" = "#6699FF")) +
  geom_errorbar(aes(ymin=Lower, ymax=Upper), width=.2,
