...
setwd("~/KayleeStuff/Smith_Lab/Data/Mouse_Validation_Data/Maternal_EPO")

#reads a file in a table format and creates a data frame from it
datafile <- "MK_EPO_MDvEtOH_For_ddCtDotPlot.csv"
...
png("MatKid_EPO_ddCtDotPlot_B&W.png", units="in", width=5, height=7, res=600)
...
p<- ggplot(dataframe_AdjCtCalcs, aes(x=Biological.Sets, y=mean)) + 
  ...
  ggtitle("Maternal Kidney") +
  ylab(expression("Erythropoietin expression relative to CON")) + 
