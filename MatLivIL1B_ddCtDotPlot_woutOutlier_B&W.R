...
setwd("~/KayleeStuff/Smith_Lab/Data/Mouse_Validation_Data/Maternal_IL1B")

#reads a file in a table format and creates a data frame from it
datafile <- "MatLivIL1B_Ctcalcs_woutOutlier_file1_For_ddCtDotPlot.csv"
...
png("MatLiv_IL1B_ddCtDotPlot_B&W.png", units="in", width=2.5, height=7, res=600)
...
p<- ggplot(dataframe_AdjCtCalcs, aes(x=Biological.Sets, y=mean)) + 
  ...
  ggtitle("IL1B") +
  ylab(expression("IL1B expression relative to CON")) + 
