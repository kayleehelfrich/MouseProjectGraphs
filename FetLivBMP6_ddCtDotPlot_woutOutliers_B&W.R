...
setwd("~/KayleeStuff/Smith_Lab/Data/Mouse_Validation_Data/Fetal_BMP6")

#reads a file in a table format and creates a data frame from it
datafile <- "FetLivBMP6_Ctcalcs_woutOutliers_file_For_ddCtDotPlot.csv"
...
png("Fetal_BMP6_ddCtDotPlot_B&W.png", units="in", width=5, height=7, res=600)
...
p<- ggplot(dataframe_AdjCtCalcs, aes(x=Biological.Sets, y=mean)) + 
  ...
  ggtitle("Fetal Bmp6 Expression") +
  ylab(expression("Bmp6 expression relative to CON")) + 
