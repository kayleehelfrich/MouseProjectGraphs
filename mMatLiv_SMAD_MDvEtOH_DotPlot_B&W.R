...
datafile <- "MatLiv_pSMAD_SMAD_TP_FinalValues.csv"
data1 <- read.csv(datafile, header=TRUE) 
dataframe_Original <- data.frame(data1)

#SMAD
png("mMativ_SMAD-TP_MDvEtOH_DotPlot_B&W.png", units="in", width=4, height=7, res=600)

order <- c("CON", "ALC")

plot <- ggplot(data= dataframe_Original, aes(x=Group, y=SMAD.Avg), fill=order) + #creates original graph
  ...
  ggtitle("SMAD5") +
  scale_y_continuous(limits = c(0,2.5), expand = c(0,0)) + #this sets the minimum and maximum values shown on the y-axis
...
# pSMAD
png("mMativ_pSMAD-TP_MDvEtOH_DotPlot_B&W.png", units="in", width=4, height=7, res=600)

order <- c("CON", "ALC")

plot <- ggplot(data= dataframe_Original, aes(x=Group, y=pSMAD.Avg), fill=order) + #creates original graph
  geom_bar(stat="identity", color="black", aes(fill = Group),
           position=position_dodge()) +
  scale_fill_manual(values=c("CON" = "grey100", "ALC" = "grey51")) +
...
# pSMAD/SMAD
png("mMativ_pSMAD-SMAD-TP_MDvEtOH_DotPlot_B&W.png", units="in", width=4, height=7, res=600)

order <- c("CON", "ALC")

plot <- ggplot(data= dataframe_Original, aes(x=Group, y=Ratio.Avg), fill=order) + #creates original graph
  geom_bar(stat="identity", color="black", aes(fill = Group),
           position=position_dodge()) +
  scale_fill_manual(values=c("CON" = "grey100", "ALC" = "grey51")) +
  geom_errorbar(aes(ymin=Ratio.Lower, ymax=Ratio.Upper), width=.2,
                position=position_dodge(2)) + 
