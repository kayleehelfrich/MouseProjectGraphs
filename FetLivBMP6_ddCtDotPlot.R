...
rm(list=ls())
library(ggplot2)
setwd("C:/Users/kaylee/Documents/Kaylee_Stuff/Smith_Lab/Data/Mouse_Validation_Data/Fetal_BMP6")

#reads a file in a table format and creates a data frame from it
datafile <- "FetLivBMP6_Ctcalcs_file1_For_ddCtDotPlot.csv"
...
png("FetLiv_BMP6_ddCtDotPlot.png", units="in", width=7, height=7, res=1000)
...
p <- ggplot(data= dataframe_AdjCtCalcs, aes(x=Biological.Sets, y=Adj_Ct, color = Biological.Sets), fill=order) + #creates original graph
  ...
  ggtitle("Fetal Liver BMP6") + #adds title
  xlab("Treatment") + #adds x-axis label
  ylab("Fetal Liver BMP6/Gapdh mRNA (fold induction)") + #adds y-axis label
