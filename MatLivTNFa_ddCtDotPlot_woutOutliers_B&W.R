...
setwd("~/KayleeStuff/Smith_Lab/Data/Mouse_Validation_Data/Maternal_TNFa")

#reads a file in a table format and creates a data frame from it
datafile <- "MatLivTNFa_Ctcalcs_woutOutliers_file1_For_ddCtDotPlot.csv"
...
png("MatLiv_Tnfa_ddCtDotPlot_B&W.png", units="in", width=2.5, height=7, res=600)
...
p<- ggplot(dataframe_AdjCtCalcs, aes(x=Biological.Sets, y=mean)) + 
  ...
  geom_point(aes(x=Biological.Sets, y=Adj_Ct),
             size = 3.5, shape = 19, position = position_jitter(width=.15, height=0)) +
  scale_y_continuous(expand = c(0,0), limits = c(0,4)) +
  ggtitle("TNFa") +
  ylab(expression("TNFa expression relative to CON")) + 
