...
setwd("~/KayleeStuff/Smith_Lab/Data/Mouse_Validation_Data/Maternal_IL6")

#reads a file in a table format and creates a data frame from it
datafile <- "MatLivIL6_Ctcalcs1woutOutlier_file1_For_ddCtDotPlot.csv"
...
png("MatLiv_IL6_ddCtDotPlot_B&W.png", units="in", width=2.5, height=7, res=600)
...
p<- ggplot(dataframe_AdjCtCalcs, aes(x=Biological.Sets, y=mean)) + 
  ...
  ggtitle("IL6") +
  ylab(expression("IL6 expression relative to CON")) + 
